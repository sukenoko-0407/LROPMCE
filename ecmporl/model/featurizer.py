...
def get_atom_feat(atom):
    atom_feat = [
        ...
        index_encoding(atom.GetValence(rdchem.ValenceType.IMPLICIT), allowable_atom_feats['atom_implicit_valence']),
...
def get_graph_data(smi):
    # Important: Allow * (dummy) in SMILES, handled in atom types
    ...
    if len(bond_index) == 0:
         # Single atom or disconnects?
         edge_index = torch.empty((2, 0), dtype=torch.long)
         edge_attr = torch.empty((0, len(allowable_bond_feats)), dtype=torch.long) # dimension mismatch?
         # bond_feats usually 4D
         edge_attr = torch.empty((0, 4), dtype=torch.long)
    else:
        edge_index = torch.tensor(bond_index, dtype=torch.long).t()
        edge_attr = torch.tensor(bond_feats, dtype=torch.long)
