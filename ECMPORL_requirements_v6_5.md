...
### 1.1 v6.5での変更点（v6.4→v6.5）

- **Value target z の定義を明確化**: `value_target_mode` により MC（per-simulation）/ MC（per-node mean）を切り替え可能に（セクション15.1）
- **PUCT の N_total を定義**: `N_total = Σ_{a∈legal} N(s,a)`（確定visitのみ、legal限定）を明記（セクション12.2.2）
- **PUCT 細部仕様を固定**: 未展開 Q_eff 初期値（0.0）、tie-break（action_id最小）を明記（セクション12.2.2）
- **探索と学習の分離を明記**: 探索セッション中は固定 checkpoint を使用する運用規約を追加（セクション16.4）

### 1.2 v6.4での変更点（v6.3→v6.4）
...
- `combine_smiles(branch_smiles_with_dummy: str, frag_smiles_no_dummy: str) -> str`
  ...
  - ただし、fragmentについては、先にcombine_smiles関数内でsmiles_addH_then_deuterateを処理する。この関数はFragmentに存在するH原子を重水素に変換する。
- `hydrogen_replace(leaf_smiles_no_dummy: str) -> list[str]`
  - Leaf に対して、次の付加位置（ダミー）を導入した候補（**ダミー1個付き**）を返す
  - 重水素部分をダミーに変換する（これによりRootの特定部分のみからのFragment伸長が可能となる）
...
再現性と挙動の安定性のため、以下の細部を固定する。

**N_total の定義**:

```text
N_total = Σ_{a ∈ legal(s)} N(s,a)
```

- legal action に対する **確定訪問回数の合計**
- `inflight` は含めない（`N_eff` ではなく `N` を使用）
- illegal action は含めない
...
- 未展開行動（`N(s,a) = 0` かつ `inflight(s,a) = 0`）の Q_eff は 0.0 とする
- これにより、prior P(s,a) が高い未展開行動が優先的に選択される

**同点時の tie-break**:

```text
tie-break: action_id が最小のものを選択（決定的）
```

- 同一スコアの action が複数存在する場合、action_id（fragment のインデックス）が最小のものを選択
- 乱数による tie-break は再現性を損なうため採用しない
...
- Value target: `z`（Value target は `value_target_mode` により定義を切り替える）

- `value_target_mode`（必須）:
  - `"A"`: MC（per-simulation）
    - 1 simulation の最終確定報酬を `R` とする。
    - 当該 simulation が通過した各状態 `s` について、学習データ (s, π, z) を生成し、`z = R` とする。
  - `"A_prime"`: MC（per-node mean）
    - Node（状態）に保持された統計 `W(s), N(s)` は「確定報酬 R の累積」とする。
    - 学習データ生成時点で、各状態 `s` について `z = W(s)/N(s)` として (s, π, z) を生成する（同一 `s` からは原則1サンプル）。
    - 推奨: `sample_weight = N(s)` を併せて保持し、学習時に重みとして用いてもよい。
...
- `sample_weight` を持つ場合、Loss は `sample_weight` により重み付けしてよい
  - 例: `L_value = sample_weight * (v - z)^2`
...
- `extract_training_samples(tree) -> Dataset`
  ...
  - value_target_modeに応じてzのサンプル粒度が変わる
...
探索フェーズと学習フェーズは明確に分離し、以下の運用規約に従う。

#### 16.4.1 探索セッション中の checkpoint 固定

```text
単一探索セッション内では、全 Worker が同一の model checkpoint を使用すること
```

- 探索開始時に checkpoint を固定し、探索終了まで更新しない
- これにより、木マージ時の統計量の整合性を保証する
- 異なる checkpoint で生成された木のマージは、統計の意味が変わるため禁止

#### 16.4.2 探索・学習サイクル

推奨される運用サイクル:

1. **探索フェーズ**: 固定 checkpoint で MCTS 探索を実行
2. **学習データ抽出**: 探索終了後、木から学習サンプルを抽出
3. **学習フェーズ**: 抽出データで model を更新し、新しい checkpoint を保存
4. **次の探索フェーズ**: 新しい checkpoint を使用して探索を再開

#### 16.4.3 checkpoint 識別（推奨）

木と checkpoint の対応を追跡するため、以下を推奨:

- `MCTSTree` に `checkpoint_id: str` を保持（checkpoint のハッシュまたはパス）
- `merge_trees()` で `checkpoint_id` の一致を検証し、不一致時は警告をログ出力
...
| value_target_mode | str | "A" | Value target の定義モード（"A"=per-simulation MC, "A_prime"=per-node mean MC） | 15.1 |
