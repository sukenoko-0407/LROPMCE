...
- PUCT は AlphaGo Zero 型の式
  ...
  \]
  をそのまま用いる。
- **連続行動空間**: 行動（Fragment選択）をK次元連続空間で表現し、ガウス分布からサンプリング。
- **分離モデル構成**: Policy Network（ガウス分布出力）とValue Network（状態価値出力）を分離、Fragment埋め込みは事前学習済みで固定。
...
- **行動 (Action)**: Fragment テーブル中の Fragment を選択し、現在の状態と結合する操作。**連続行動空間**においてK次元ベクトルとして表現される。
- **コア構造**: 探索の起点となるダミーアトム付き SMILES（例: `*c1ccccc1`）。
- **Fragment**: ダミーアトム付き SMILES（例: `*CCN`）と、その物性情報（HAC, cnt_hetero, cnt_chiral, MW）を持つ単位構造。各FragmentはK次元空間における固定座標を持つ。
...
1. **PyTorch モデル**（分離型構成）
   ...
   本システムでは、行動を**連続空間**で扱う。以下の3つのモデルコンポーネントを使用する。
   ...
   - **出力**: K次元ガウス分布のパラメータ
     - `mean`: K次元ベクトル（分布の平均）
     - `std_log`: K次元ベクトル（分布の標準偏差の対数）
   - **役割**: 状態に基づいて、行動空間（K次元連続空間）における確率分布を定義する。
   - **シグネチャ**: `PolicyNetwork(state_smiles: str) -> Tuple[Tensor[K], Tensor[K]]`
   - **学習方法**:
     - **ターゲット分布の生成**: 子ノードのq_valueを重みとしてsoftmax正規化し、ターゲット確率分布を作成
     - **Loss関数**: Policy Networkが出力するガウス分布と、ターゲット確率分布のKLダイバージェンスを最小化
     - **学習対象**: 評価済み（`leaf_calc == "evaluated"`）の子ノードのみ。未評価ノードは学習対象外

   #### 1.2 Fragment Embedding Model

   - **入力**: FragmentのSMILES
   - **出力**: K次元空間における座標ベクトル
   - **役割**: 各FragmentをPolicy Networkと同じK次元空間に埋め込む。
   - **シグネチャ**: `FragmentEmbedding(fragment_smiles: str | list[str]) -> Tensor[K] | Tensor[M, K]`
   - **重要**: **事前学習済みで固定**。探索中は学習しない。将来的なfine-tuningは別途検討。

   #### 1.3 Value Network

   - **入力**: 状態（ダミーアトム付きSMILES）
   - **出力**: その状態における期待平均報酬（スカラー）
   - **役割**: MCTSのバックアップでQ(s,a)を更新するために使用。
   - **シグネチャ**: `ValueNetwork(state_smiles: str) -> float`
   - **学習ターゲット**: Tree上のノードの`q_value`（平均報酬）。

   #### 1.4 行動選択の仕組み（連続行動空間）

   ```text
   状態 s → Policy Network → (mean, std_log) : K次元ガウス分布

   Legal Actions (M個のFragment) → Fragment Embedding → coords[M, K] : 各Fragmentの固定K次元座標

   各Fragmentの確率密度:
     pdf_i = N(coords[i] | mean, exp(std_log))  # K次元ガウス分布のPDF

   正規化された選択確率:
     P(a_i | s) = pdf_i / Σ_j pdf_j
   ```

   - **行動の選択**: Legal Action内の各Fragmentについて、Policy Networkが出力するガウス分布における確率密度を計算し、全Legal Actionで正規化して確率とする。
...
2. **Simulation（1回）**: 以下の3フェーズからなる。
   ...
   **Phase 2: Expansion**
   ...
   - **UCTモード**: 未選択のFragmentがあれば優先的に選択。全て選択済みならUCTスコアで選択。
   ...
   - 各次状態候補に対して Value Network による softmax サンプリングで 1 状態を選び、新ノードを生成・Tree に追加。
   - 新ノードについて以下を実施:
   ...
   - `leaf_calc == "ready"` の場合、**評価待ちキューに追加**し、`leaf_calc = "pending"` に更新。
...
1. **状態情報**
   ...
   - `leaf_calc`: Leaf評価の状態を記録する。以下の**4つ**の状態を持つ:
     ...
     - `"ready"`: Leaf評価の条件を満たしているが、まだキューに追加されていない。
     - `"pending"`: 評価待ちキューに追加済み。**Selection対象外**。
     - `"evaluated"`: すでにLeaf評価を実行済み。
...
3. **構造情報**
   ...
   - `parent`: 親ノードへの参照（root の場合は None）。**最初に到達した経路の親のみを保持**。
...
2. **木構造制御メソッド**
   ...
   #### 2.6 `backpropagate(node, reward)`

   - 指定されたノードからrootまで遡り、経路上の全ノードの統計量を更新。
   - 更新内容:
     - `visit_count (N)` += 1
   ...
   #### 2.7 `backpropagate_batch(node_reward_pairs: list[Tuple[MCTSNode, float]])`

   - 複数のノードに対するBackpropagationを**バッチ処理**する。
   ...
   ```python
   pending_nodes = []  # 評価待ちキュー

   for i in range(n_simulations):
       # Selection: pending以外のノードを辿る
       node = traverse(stop_condition)
       ...
       if expandable(node):
           ...
           # 評価対象の判定
           if new_node.leaf_calc == "ready":
               pending_nodes.append(new_node)
               new_node.leaf_calc = "pending"
           elif new_node.is_terminal and new_node.depth < min_depth:
               # 行き詰まり: 即時報酬0でBackpropagate
               backpropagate(new_node, 0.0)

       # バッチ評価トリガー
       if len(pending_nodes) >= batch_eval_interval:
           rewards = evaluate(pending_nodes)
           backpropagate_batch(zip(pending_nodes, rewards))
...
4. **状態重複の取り扱い（トランスポジション）**
   ...
   - **親ノードは最初に到達した経路のものを維持**。後からの到達では親を更新しない。
   - Backpropagation時は、そのSimulationで通った経路（最初に記録された親）を遡る。

5. **学習データ抽出支援**

   - Value Network 学習用: 全ノードまたは条件を満たすノードから `(state_smiles, q_value)` ペアを抽出するメソッドを提供。
   - Policy Network 学習用: 子ノードを持つノードから `(parent_state_smiles, children_actions, children_q_values)` を抽出するメソッドを提供。
...
1. **Fragment 管理**
   ...
   - 初期化時に、`fragment_featurizer` を用いて全 Fragment のグラフ表現を前計算し、Fragment Embedding Model 入力に利用できる形でキャッシュする。
...
2. **保持する要素（nn.Moduleのサブモジュール）**
   ...
   - `policy_network`: Policy Network（ガウス分布パラメータ出力）
   - `value_network`: Value Network（状態価値出力）
   - **注意**: `fragment_embedding` は**Environmentが保持**（固定のため学習対象外）
...
3. **学習データの抽出と学習**
   ...
   - **Policy Networkの学習データ**:
     - 子ノードを持つノードから `(parent_state_smiles, children_actions, children_q_values)` を抽出。
     - **`leaf_calc == "evaluated"` の子ノードのみ**が対象。未評価ノードは含まない。
     - ターゲット分布: 子ノードのq_valueをsoftmax正規化した確率分布
   - **Loss関数**:
     - Value Network: `L_value = (v - z)²` （v: 出力, z: value_target）
     - Policy Network: ガウス分布とターゲット分布のKLダイバージェンス

4. **PUCT 用 P(s,a) の定義（連続行動空間）**

   - Policy Networkが出力する(mean, std_log)から、各Legal ActionのK次元座標における確率密度を計算。
   - 確率密度を全Legal Actionで正規化し P(s,a) とする。
   - 計算式:

   ```text
   pdf_i = Π_k N(coord_i[k] | mean[k], exp(std_log[k]))
   P(s, a_i) = pdf_i / Σ_j pdf_j
   ```

5. **V(s) による次状態選択**

   - `combine_smiles` → `hydrogen_replace` により複数の次状態候補 `{s'_i}` が生成された際、
     - 各候補に対して Value Network 出力 v(s'_i) を計算。
     - 温度付き softmax:
       \[
...
7. **モデルの保存・読み込み**
   ...
   - PyTorch の state_dict を用いて、Policy Network と Value Network の重みを保存・読み込みできる API を提供。
...
2. **PUCT モード（連続行動空間対応）**
   ...
   - PUCT 式:
     ...
     \]
   - P(s,a) は Policy NetworkとFragment Embeddingから計算される行動確率（K次元ガウス分布の正規化確率密度）を用いる。
...
3. **Backpropagationの更新対象**
   ...
   - Backpropagate時は、**そのSimulationで実際に通った経路**を遡る。
   - トランスポジションにより既存ノードを再利用した場合でも、最初に記録された親経路を辿る。
...
2. **モデル重みの保存**
   ...
   - Fragment Embeddingは固定のため、別途保存（または毎回計算）。
...
```python
def backpropagate_batch(node_reward_pairs: list[Tuple[MCTSNode, float]]):
    # 各ノードへの更新を集計
    updates = defaultdict(lambda: {"reward_sum": 0.0, "visit_delta": 0})

    for node, reward in node_reward_pairs:
        current = node
        while current is not None:
            key = (current.state_smiles, current.depth)
...
- 探索完了後、全Workerの木をマージして1つの大きな木を形成。
...
1. **拡張性**
   ...
   - **モデル構成の拡張性**: Policy Network, Value Networkを別々に差し替え可能。
...
- `Environment`:
  - `Environment(fragment_df, combine_fn, hydrogen_replace_fn, reward_fns, alert_fn, calc_props_fn, count_subspace_fn, fragment_featurizer, fragment_embedding_model, constraints)`
  - `Environment.evaluate_batch(smiles_list: list[str]) -> list[float]`
  - `Environment.get_legal_actions(state_smiles: str) -> list[str]`
  - `Environment.get_fragment_embeddings(fragment_smiles: list[str]) -> Tensor[M, K]`

- `Agent(nn.Module)`:
  - `Agent(policy_network, value_network, config)`
  ...
  - `Agent.train_step(tree: MCTSTree) -> dict`  # Loss等を返す
...
- `MCTSTree`:
  ...
  - `tree.backpropagate_batch(node_reward_pairs: list[Tuple[MCTSNode, float]])`
...
以上が ECMPORL_02 パッケージの要件定義書 v3.1。
