...
- **未展開行動があれば**：未展開集合から 1 つ選択する  
  - 推奨: `seed` による擬似乱数で一様サンプリング（探索バイアスを減らす）  
  - 再現性を最優先する運用では `action_id` 昇順の最小を選ぶモードも用意してよい（`tie_break_mode`）

- **全て展開済みなら**：以下の UCT スコアを最大化する（pending child は除外）
  - 定義（確定統計 / completed のみ）  
    - `Q(s,a) = W(s,a) / max(1, N(s,a))`  
    - `N_total(s) = sum_{a∈legal} N(s,a)`  
    - `score(s,a) = Q(s,a) + c_uct * sqrt( ln(N_total(s) + 1) / N(s,a) )`
  - `ln` は自然対数（底 *e*）。未訪問 `N(s,a)=0` は **+∞ と同等**として先に処理する（上式の分母 0 を避ける）

- **virtual loss を考慮する場合（推奨）**：selection 時のみ `eff` 統計を使う  
  - `N_eff = N + inflight`  
  - `W_eff = W - vloss * inflight`（vloss>0 を前提）  
  - `Q_eff = W_eff / max(1, N_eff)` として score を計算  
  - backprop による確定更新は **completed 統計のみ**を更新する（inflight は flush 完了で解消）

UCT の係数 `c_uct` は `SearchConfig` に追加し、デフォルト値（例: 1.0）を実装側で定める。
...
- 対象: `leaf_calc="done"` の Leaf
- `visit_count` は **Leaf 状態の確定訪問回数 `N(s)`（completed のみ）**とする（transposition / merge を跨いで集約できるため）
- 参考: デバッグ用途に `visit_count_effective = N(s) + inflight(s)` を併記してもよい（selection の混雑度を見るため）
- 出力: `LeafResult(leaf_smiles, value, visit_count, depth, mol_props)`
...
- 1) **UCT の具体式**：標準UCTを採用（10.2 節の式）。log は自然対数、`N_total(s)=sum_a N(s,a)`、未訪問は優先。
...
- 3) **visit_count の定義（推論専用モードの出力）**  
  - 本仕様では `visit_count = Leaf の N(s)（completed）` を採用する案に更新済み（15章）。この定義で確定してよいか。
