...
`action_id` は 0..K-1 を前提にする。欠番がある場合は **読み込み時に再index**し、元IDを保持する（運用でどちらを採用するかは要確認）。
...
- **未展開行動があればランダムに1つ**
- 全て展開済みなら UCT スコア最大
- virtual loss を考慮する場合、Q_eff / N_eff を使う（探索項は通常 `sqrt(log(N_total)/N_eff)`）

UCT の具体式は要件で固定されていないため、実装側で標準式を採用し、式を docstring に明記する。
...
- `visit_count` は（暫定）**親Branchの該当actionの N(s,a) 最大値**、または Leaf 到達回数カウンタ（実装でどちらにするか要確定）
...
実装着手前に、以下のどれを採用するか決めると手戻りが減ります。

1. **UCT の具体式**（標準UCTで良いか、Qの定義、探索項のlog基底、N_totalの扱い）
2. **leaf_only（Branch候補が空）**のときの扱い  
   - max_depth でなくても報酬計算して終了するか / 0で終了するか
3. **visit_count の定義**（推論専用モードの出力）
4. Fragment CSV の `action_id` が 0..K-1 連番か（欠番あり得るか）
