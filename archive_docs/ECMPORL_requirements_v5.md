...
2. **Simulation（1回）**: 以下の3フェーズからなる。
   ...
   **Phase 2: Expansion**
   ...
   - **PUCTモード**: Policy Networkから `policy_logits`（Fragmentごとのlogit）を取得し、Legal Actionでマスク → softmaxで確率化（温度τ適用可）して選択。
   ...
   - **`leaf_calc` に応じた処理**:
     - `leaf_calc == "ready"`: **評価待ちキューに追加**し、`leaf_calc = "pending"` に更新した上で、このSimulationを終了する。経路上に `inflight += 1` を付与する（virtual loss）。
     - `leaf_calc == "not_ready"`: 評価は行わず、**同一Simulation内で**このノードを起点として行動選択＋展開を継続する（1回のSimulationで複数回の展開が発生し得る）。

   **Phase 3: （バッチ評価後）Backpropagation**
   - バッチ評価がフラッシュされたタイミングで実行（後述の6.10参照）。
   - Leaf で得た報酬（合計報酬）を root までの経路上のノードに逆伝播。

3. 上記シミュレーションを `max_simulation` 回繰り返す。

4. **バッチ評価フラッシュ**: 評価待ちキューが `eval_batch_size` に到達 / `max_wait_ms` 超過 / `batch_eval_interval` 到達 / Selectionが全pendingで行き詰まり、のいずれかでバッチ評価を実行し、完了次第 Backpropagation を実施する（6.6, 6.7, 6.10参照）。
...
3. **構造情報**
   ...
   - `children`: 行動（Fragment）ごとの子要素を保持する辞書。各要素は少なくとも `(child_node, inflight(s,a), prior_P(s,a) [puct時])` を含む **Edge統計** を保持できる構造とする（virtual loss のため `inflight` が必要）。
...
2. **行き詰まりの扱い**
   ...
   - `leaf_calc == "not_ready"`（Leaf評価条件未到達） かつ以下のいずれかの場合:
     - 制約を満たす Fragment が存在しない（合法手が空）。
...
以上が ECMPORL_02 パッケージの要件定義書 v5.0。
